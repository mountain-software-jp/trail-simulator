...
### **GUIアプリケーション使用 (Electron)**

ElectronベースのGUIアプリケーションを使用することで、より直感的に操作できます。
...
1. **セットアップタブ**: GPXファイルを選択してコースデータを作成し、パラメータを設定
2. **シミュレーションタブ**: シミュレーションを実行
3. **結果タブ**: 分析を実行し、生成されたグラフとアニメーションを表示

GUIアプリケーションでは、すべての操作がウィンドウ内で完結し、結果の可視化も自動的に行われます。
